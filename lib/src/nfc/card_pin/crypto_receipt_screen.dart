      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            CachedNetworkImage(
              imageUrl: widget.payModel.data?.merchantLogo ?? '',
              height: size.height * 0.08,
            ),
            const SizedBox(height: 2.0),
            Center(
              child: Text(
                (widget.payModel.data?.merchantName ?? '')
                    .replaceAll('    ', ' '),
            ...
            Center(
              child: Text(
                '${widget.payModel.data?.merchantAddress?.address ?? ''} ${widget.payModel.data?.merchantAddress?.city ?? ''} ${widget.payModel.data?.merchantAddress?.state ?? ''} ${widget.payModel.data?.merchantAddress?.country ?? ''} ${widget.payModel.data?.merchantAddress?.zip ?? ''}'
                    .replaceAll('    ', ' '),
                textAlign: TextAlign.center,
                style: const TextStyle(
                  fontSize: 12.0,
            ...
            _listBuilder(
              size: size,
              title: 'Date Time',
              description: '${widget.payModel.data?.date ?? ''}',
            ),
            _listBuilder(
              size: size,
              title: 'merchant Id',
              description: widget.payModel.data?.merchantId ?? '',
            ),
            _listBuilder(
              size: size,
              title: 'card Type',
              description: widget.payModel.data?.cardType ?? '',
            ),
            _listBuilder(
              size: size,
              title: 'merchant Name',
              description: widget.payModel.data?.merchantName ?? '',
            ),
            _listBuilder(
              size: size,
              title: 'name On Card',
              description: widget.payModel.data?.nameOnCard ?? '',
            ),
            _listBuilder(
              size: size,
              title: 'transaction Id',
              description: widget.payModel.data?.transactionId ?? '',
            ),
            _listBuilder(
              size: size,
              title: 'card Masked',
              description: widget.payModel.data?.cardMasked ?? '',
            ),
            _listBuilder(
              size: size,
              title: 'Crypto Amount',
              description: '${widget.payModel.data!.crypto} ${widget.payModel.data!.cryptoamount}',
            ),
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    '${widget.payModel.data!.currency} ${widget.payModel.data!.fiatamount}',
                    style: const TextStyle(
                      fontSize: 16.0,
                      fontWeight: FontWeight.w600,
                    ),
          ...
          return pw.Column(
            ...
            children: [
              ...
              pw.Padding(
                ...
                child: pw.Text(
                  widget.payModel.data?.disclaimer ?? '',
                  textAlign: pw.TextAlign.center,
