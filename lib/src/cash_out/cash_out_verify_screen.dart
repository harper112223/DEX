...
class CashOutVerifyOTPScreen extends StatefulWidget {
  final CardScanResponse cardScanResponse;
  final String amount;
  const CashOutVerifyOTPScreen({
    Key? key,
  ...
  @override
  State<CashOutVerifyOTPScreen> createState() => _CashOutVerifyOTPScreenState();
}

class _CashOutVerifyOTPScreenState extends State<CashOutVerifyOTPScreen> {
  String _otpCode = "";
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        title: "Cash Out Verify OTP",
        backgroundColor: ColorName.primaryRed,
        textColor: Colors.white,
      ),
      body: BlocConsumer<CashOutVerifyOTPCubit, CashOutVerifyOTPState>(
        buildWhen: (previous, current) {
          if (current is CashOutVerifyOTPSucssState) {
            return false;
          } else {
            return true;
          }
        ...
        listener: (context, state) {
          if (state is CashOutVerifyOTPSucssState) {
          ...
          } else if (state is CashOutEmitSucssState) {
            state.message.showSnackBar(context);
            Navigator.of(context).pushNamed(
          ...
          } else if (state is CashOutVerifyOTPErrorState) {
            setState(() => _otpCode = '');
            state.errorMessage.showSnackBar(context);
          } else if (state is CashOutResendOTPSucsssState) {
            setState(() {
              _otpCode = '';
              _start = 15;
              startTimer();
        ...
        builder: (context, state) {
          if (state is CashOutVerifyOTPLoadingState) {
